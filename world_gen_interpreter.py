...
class WorldGenInterpreter:
    ...
    TO USE: use method translateLevelCode(), then get tile_group, enemy_group, npc_group.
    ...
    Attributes:
        ...
        tile_group (pygame.sprite.Group): Group that represents the tile objects for the current level.
    ...
    Methods:
        ...
        addEntityToGroup(self, tile_code): Adds tile to tile_group, and any additional entities to their respective groups
        translateLevelCode(self): Main function which carries out the translation of level code, and adds objects to tile_group, enemy_group, npc_group
    """
    ...
    # Constructor
    def __init__(self, world_gen_file, enemy_id_file, npc_id_file, level_name, tile_group=pygame.sprite.Group, enemy_group=pygame.sprite.Group, npc_group=pygame.sprite.Group):
            ...
            with open(self.getEnemyIdFile(), 'r') as enemy_file:
                ...
                for line in file_lines:
                    if str_to_find in line: # if line contains enemy id
                        enemy_info = [i for i in line.split()[1].split('_')] # Enemy information split into a list:
            ...
            with open(self.getNpcIdFile, 'r') as npc_file:
                ...
                for line in file_lines:
                    if str_to_find in line: # if line contains npc id
                        npc_info = [i for i in line.split()[1].split('_')] # Npc information split into a list:
    ...
    def addEntityToGroup(self, tile_info):
        ...
        Interprets tile_info, then adds tile entity to tile_group.
        ...
        # Adding tile to tile_group
        if tile_type == 'G':
            self.getTileGroup().add(Tile('grass', xcoord, ycoord))
        elif tile_type == 'W':
            self.getTileGroup().add(Tile('wall', xcoord, ycoord))
